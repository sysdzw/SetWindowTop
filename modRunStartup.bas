...
Public Sub setAutoRun()
    ...
    Else
        fname = App.Path & "\"
    End If
    SaveString HKEY_CURRENT_USER, "Software\Microsoft\Windows\CurrentVersion\Run", "SetWindowTop", fname & App.EXEName & ".exe "
'    MsgBox "建立开机启动成功！"
End Sub
...
Public Sub cancelAutoRun()
    ...
    RegDeleteValue hKey, "SetWindowTop"    ' 删除键值
    RegCloseKey hKey    ' 关闭句柄
End Sub
Public Function isHasSetAutoRun() As Boolean
    isHasSetAutoRun = (GetString(HKEY_CURRENT_USER, "Software\Microsoft\Windows\CurrentVersion\Run", "SetWindowTop") <> "")
End Function
